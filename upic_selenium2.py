...
MIN_PARSED_PAGES_COUNT = 80
MAX_PARSED_PAGES_COUNT = 90
...
class UpicBaseScrapper:
    ...
    def go_to_detail_pages(self):
        # for each upic in input_upic.txt
        with open("input_upic.txt") as input_upic_file_handler:
            company_upic_list = input_upic_file_handler.read().split("\n")
        for company_upic in company_upic_list:
            self._apply_search(company_upic)
            self._get_to_detail_page()

    def _apply_search(self, company_upic):
        # select state
        self.selenium.find_element_by_id('plcContent_ctl00_txtUPIC').send_keys(company_upic)
            ...
            if "Primary Contact" in self.selenium.find_element_by_xpath(
                ...
                for i in range(len(
                    self.selenium.find_elements_by_xpath(
                        '//*[@id="plcContent_ctl00_contactPanel"]/fieldset/table/tbody/tr/td[2]/table/tbody/tr'
                    )
                )):
                    if "Email" in self.selenium.find_element_by_xpath(
                        '//*[@id="plcContent_ctl00_contactPanel"]/fieldset/table/tbody/tr/td[2]/table/tbody/tr[{}]/td[1]'.format(i + 1)
                    ).text:
                        self.primary_contact_email_address = self.selenium.find_element_by_xpath(
                            '//*[@id="plcContent_ctl00_contactPanel"]/fieldset/table/tbody/tr/td[2]/table/tbody/tr[{}]/td[2]'.format(i + 1)
                        ).text
                        print("------------------------------------------------------------------------")
    ...
    def _csv_writer(self):
        ...
        # reset all the values again after writing to the csv
        self.company = ''
        self.ppai = ''
        self.ppai_credit = ''
        self.epsa = ''
